# TC-028: GET /api/v1/reports/comparison
...
## 影响范围
此BUG导致GET /api/v1/reports/comparison功能完全无法使用，严重影响用户体验和系统可用性。
...
## 详细错误日志
```
对比分析报表API集成测试 › GET /api/v1/reports/comparison › 正常场景 - 获取对比分析报表成功
    ...
    > 37 |       expect(response.status).toBe(200);
      ...
      39 |       expect(response.body.data).toHaveProperty('comparisonData');
      40 |     });

      at Object.toBe (tests/in...
```
