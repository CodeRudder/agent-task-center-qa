# TC-056: GET /api/v1/agents

## 测试信息
- **TC编号**：TC-056
- **API路径**：Unknown API
- **优先级**：🔴 P0
- **测试文件**：获取Agent列表API集成测试
...
## 影响范围
此BUG导致GET /api/v1/agents功能完全无法使用，严重影响用户体验和系统可用性。
...
## 详细错误日志
```
获取Agent列表API集成测试 › GET /api/v1/agents › 正常场景 - 获取Agent列表成功
    ...
    > 33 |       expect(response.status).toBe(200);
      ...
      35 |       expect(response.body.data).toHaveProperty('agents');
      36 |       expect(...
```
